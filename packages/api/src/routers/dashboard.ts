...
export const dashboardRouter = {
  ...
  getStats: staffProcedure.handler(async ({ context }) => {
    ...
    const businessFilter = sql`${client.businesses} && ARRAY[${sql.join(
      accessibleBusinesses.map((b) => sql`${b}`),
      sql`, `
    )}]::text[]`;
    const matterBusinessFilter = sql`${matter.business}::text = ANY(ARRAY[${sql.join(
      accessibleBusinesses.map((b) => sql`${b}`),
      sql`, `
    )}]::text[])`;
    const deadlineBusinessFilter = sql`(${deadline.business} IS NULL OR ${deadline.business}::text = ANY(ARRAY[${sql.join(
      accessibleBusinesses.map((b) => sql`${b}`),
      sql`, `
    )}]::text[]))`;
